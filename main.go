...
func main() {
	var (
		...
		listenerThreads = kingpin.Flag("udp-listener.threads", "The number of listener threads to receive UDP traffic.").Default("4").String()
	)
	...
	if *statsdListenUDP != "" {
		...
		go ul.Listen(*listenerThreads, events)
	}
